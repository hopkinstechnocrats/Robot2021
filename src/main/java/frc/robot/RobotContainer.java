...
public class RobotContainer {
        ...
        public RobotContainer() {
                ...
                m_robotDrive.setDefaultCommand(
                                ...
                                new RunCommand(() -> m_robotDrive.tankDrivePercentOutput(
        ...
        public void initializeAutoLog() {
                ...
                BadLog.createValue("DriveConstants/WheelDiameterMeters",
                ...
                BadLog.createValue("DriveConstants/kvVoltSecondsPerMeter",
                ...
                // Gyro Data
                BadLog.createTopic("NavX/WorldLinearAccelX", "m/s^2",
                                () -> (double) m_robotDrive.navX.getWorldLinearAccelX());
                BadLog.createTopic("NavX/WorldLinearAccelY", "m/s^2",
                                () -> (double) m_robotDrive.navX.getWorldLinearAccelX());
                BadLog.createTopic("NavX/WorldLinearAccelZ", "m/s^2",
                                () -> (double) m_robotDrive.navX.getWorldLinearAccelX());
                ...
                BadLog.createTopicStr("Drivetrain/Motor Faults", BadLog.UNITLESS, m_robotDrive::getMotorFaultsStr,
                                "log");
        ...
        private void configureButtonBindings() {
                ...
                new JoystickButton(m_driverController, Button.kA.value).whileHeld(new RunCommand(
                                () -> m_launcherSubsystem.spinLauncher(LauncherConstants.speed), m_launcherSubsystem));

                new JoystickButton(m_driverController, Button.kX.value)
                                .whileHeld(new SpinLauncherCommand(m_launcherSubsystem));

                // new JoystickButton(m_driverController, Button.kY.value)
                // .whenPressed(new RunCommand(() -> m_robotDrive._Orchestra.play(),
                // m_robotDrive));

                m_launcherSubsystem.setDefaultCommand(
                                new RunCommand(() -> m_launcherSubsystem.spinLauncher(0), m_launcherSubsystem));

                new JoystickButton(m_driverController, Button.kY.value).whileHeld(() -> m_PreLaunch.spin(-1),
                                m_PreLaunch);

                new JoystickButton(m_driverController, Button.kB.value).whileHeld(() -> m_PreLaunch.spin(1),
                                m_PreLaunch);

                m_intakeSubsystem.setDefaultCommand(new RunCommand(() -> m_intakeSubsystem.spin(0), m_intakeSubsystem));

                new JoystickButton(m_driverController, Button.kB.value).whileHeld(new RunCommand(() -> {
                        m_intakeSubsystem.spin(-1);
                        System.out.println("RUNNING INTAKE COMMAND");
                }));

                new JoystickButton(m_driverController, Button.kBumperRight.value).whenPressed(new InstantCommand(() -> {
                        m_intakeSubsystem.toggle();
                        System.out.println("RUNNING INTAKE DEPLOY COMMAND");
                }));

                // JoystickButton xButton = new JoystickButton(m_driverController,
                // Button.kBumperLeft.value);
                // xButton.whenPressed(InterstellarAccuracyDriveCommand.getInstance(xButton,
                // m_robotDrive, leftPIDController, rightPIDController));
        }
        ...
        public Command getAutonomousCommand(ArrayList<Trajectory> Course) {
                ...
                PIDController pidController = new PIDController(DriveConstants.kPDriveVel, 0,
